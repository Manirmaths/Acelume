...
class CandidateEntryIn(BaseModel):
    """Both optional -- a school may hand out slips without naming anyone."""
    full_name: Optional[str] = None
    # The school's OWN identifier, so results map back to their class list.
    school_reference: Optional[str] = None


class AddCandidatesIn(BaseModel):
    """
    Either a count (entrance exam, nobody named yet) or a named list.

    Registration numbers are always generated by Acelume, never supplied --
    see app/exams.py::issue_registration_number.
    """
    count: int = Field(default=0, ge=0, le=500)
    candidates: list[CandidateEntryIn] = []
...
class SubjectAvailabilityOut(BaseModel):
    subject: str
    available: int
...
class ReadinessOut(BaseModel):
    ...
    Problems rather than a boolean, so the review screen can show a checklist
