...
class IssuedRegistration(Base):
    """
    Every registration number ever issued, kept forever.

    A separate ledger rather than relying on a unique index over
    ExamCandidate, because a number must never be reissued EVEN IF the
    candidate row is later deleted or the whole session is removed. Deleting a
    session must not quietly free a number back into circulation -- a
    registration number that turns up on two different papers years apart is
    the sort of thing that discredits an exam system entirely.

    Rows are never deleted from here.
    """
...
class ExamCandidate(Base):
    ...
    The registration number is GENERATED by Acelume, not supplied by the
    school: ten characters, one letter, seven digits, two letters
    (A1234567BC). It is unique across every exam ever run -- see
    IssuedRegistration.

    Generating it rather than accepting the school's own numbering avoids two
    problems: two schools both numbering candidates from 001, and a student
    who knows the pattern guessing a classmate's number. The school's own
    identifier lives in `school_reference`, so they can still map results back
    to a class list.

    Pairing the number with a secret access code is what stops a student
    sitting the paper under someone else's identity.
    ...
    registration_number: Mapped[str] = mapped_column(String(10), nullable=False, index=True)
    ...
    # The school's OWN identifier: admission number, class list position,
    # whatever they already use. Acelume never interprets it; it exists purely
    # so the school can match a result back to a pupil.
    school_reference: Mapped[str | None] = mapped_column(String(60), nullable=True)
    ...
    __table_args__ = (
        # Registration numbers are globally unique via IssuedRegistration, so
        # this only guards against a double-insert within one session.
